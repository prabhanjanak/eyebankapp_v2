...
export function Sidebar({ mobileMenuOpen, setMobileMenuOpen }: { mobileMenuOpen?: boolean, setMobileMenuOpen?: (v: boolean) => void }) {
  ...
  const { hasNewCall } = useNotifications();
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "bg-white/80 backdrop-blur-2xl border-r border-gray-200/60 flex flex-col h-[100dvh] transition-all duration-300 select-none shadow-[4px_0_24px_rgba(0,0,0,0.01)] fixed md:relative z-50",
          collapsed ? "w-20" : "w-64",
      ...
      >
        <div className="h-16 flex items-center justify-center border-b border-gray-200/60 px-4 overflow-hidden bg-white/30">
          {collapsed ? (
            <img
              ...
              alt="SEFI"
              className="h-11 w-11 object-contain hover:scale-105 transition-transform drop-shadow-md"
            />
          ) : (
            <div className="flex items-center gap-2.5 w-full">
              ...
              <div className="leading-tight">
                <p className="text-[13px] font-extrabold text-gray-800 tracking-tight whitespace-nowrap">Netrasetu</p>
                <p className="text-[9.5px] font-bold text-orange-500 uppercase tracking-wider">Sankara Eye Bank</p>
              </div>
            </div>
          )}
        </div>
        ...
        <button
          ...
          className="hidden md:flex absolute -right-3 top-20 bg-white border border-gray-200/70 rounded-full p-1.5 text-gray-400 hover:text-[#ff7a18] hover:scale-110 hover:shadow-md transition-all duration-300 shadow-sm z-10 cursor-pointer"
        >
          ...
          {filteredNav.map((item) => {
            ...
            >
              ...
              <div className="relative flex items-center justify-center">
                ...
                />
                {item.href === "/eye-calls" && hasNewCall && (
              ...
              {!collapsed && (
                <span className="flex-1 flex items-center justify-between">
                  ...
                  {item.href === "/eye-calls" && hasNewCall && (
                    <span className="bg-red-500 text-white text-[10px] font-extrabold px-1.5 py-0.5 rounded-full animate-pulse tracking-wide">
                      NEW ALERT
                    </span>
                  )}
                </span>

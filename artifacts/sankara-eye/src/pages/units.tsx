...
import { Plus, MapPin, Phone, Building, Edit2, Trash2, AlertTriangle, Mail } from "lucide-react";
...
export default function Units() {
  ...
  const validateForm = (): boolean => {
    ...
    const email = formState.coordinatorEmail.trim();
    if (!email) {
      setFormError("Coordinator email is required");
      return false;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        coordinatorEmail: formState.coordinatorEmail.trim().toLowerCase(),
                ...
                <div className="space-y-3 bg-white/60 p-4 rounded-2xl border border-gray-200/50 shadow-sm">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-gray-400 text-[9px] uppercase font-bold tracking-wider">Email Address</div>
                      <div className="font-extrabold text-gray-800 truncate">{unit.coordinatorEmail}</div>
                    </div>
                  </div>
                </div>
                {isAdmin && (
        ...
        <DialogContent className="max-w-[480px] rounded-2xl bg-white border border-gray-150">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="unit-email" className="text-sm font-semibold text-gray-700">
                  Email Address
                </Label>
                <Input
                  id="unit-email"
                  placeholder="e.g. rajesh@sankaraeye.com"
                  type="email"
                  value={formState.coordinatorEmail}
                  onChange={(e) => setFormState({ ...formState, coordinatorEmail: e.target.value })}
                  className="bg-white border-gray-200 text-gray-900 focus:ring-2 focus:ring-[#ff7a18] focus:border-transparent rounded-lg"
                  disabled={isSaving}
                  required
                />

...
import { useListEyeCalls, useUpdateEyeCallStatus, useUpdateEyeCall, useDeleteEyeCall, getListEyeCallsQueryKey } from "@workspace/api-client-react";
...
export default function EyeCalls() {
  ...
  const updateCall = useUpdateEyeCall();
  const deleteCall = useDeleteEyeCall();

  const [thirdPartyHospital, setThirdPartyHospital] = useState("");
  const [thirdPartyHelper, setThirdPartyHelper] = useState("");
  const [isThirdPartyDialogOpen, setIsThirdPartyDialogOpen] = useState(false);

  const handleDeleteCall = (id: number) => {
    if (!confirm("Are you sure you want to delete this eye call? This action cannot be undone.")) return;
    ...
    deleteCall.mutate(
      { id },
      {
        onSuccess: () => {
          ...
          toast({ title: "Eye call deleted successfully" });
    ...
    updateStatus.mutate(
      { id, data: { status: newStatus } },
      {
        onSuccess: async () => {
          ...
          let updatedCall = selectedCall;
          if (selectedCall && selectedCall.id === id) {
            setSelectedCall((prev: any) => ({ ...prev, status: newStatus }));
            updatedCall = { ...selectedCall, status: newStatus };
          }

          // Trigger WhatsApp dispatch for team_sent
          if (newStatus === "team_sent") {
            const call = data?.data.find(c => c.id === id) || updatedCall;
            if (call && call.referrerMobile) {
              ...
              if (savedPhoneId && savedToken && savedTemplate) {
                try {
                  const targetPhone = call.referrerMobile.replace(/[^0-9]/g, "");
                  // Optional: Log to console during testing
                  console.log(`Dispatching WhatsApp to ${targetPhone} via template ${savedTemplate}`);
                  
                  const response = await fetch(`https://graph.facebook.com/v20.0/${savedPhoneId}/messages`, {
                    method: "POST",
                    headers: {
                  ...
                  if (response.ok) {
                    toast({
                      ...
                      description: `Automated retrieval team alert sent to ${call.referrerMobile}`,
                    });
                  } else {
                ...
                toast({
                  title: "⚠️ WhatsApp Not Configured",
                  description: "Please configure WhatsApp API settings to enable automated dispatch alerts.",
                  variant: "default"
                });
              }
            }
  ...
  const handleAssignThirdParty = () => {
    if (!selectedCall) return;
    updateCall.mutate(
      {
        id: selectedCall.id,
        data: {
          thirdPartyHospitalDetails: thirdPartyHospital,
          thirdPartyHelperContact: thirdPartyHelper,
        },
      },
      {
        onSuccess: () => {
          ...
          toast({
            ...
            description: "Relative has been notified via SMS/WhatsApp.",
            variant: "default",
          });
          // Update selected call locally to reflect changes immediately
          setSelectedCall({
            ...selectedCall,
            thirdPartyHospitalDetails: thirdPartyHospital,
            thirdPartyHelperContact: thirdPartyHelper,
  ...
  return (
    <>
    <div className="space-y-6 select-none animate-fadeIn print:hidden">
                ...
                data.data.map((call) => (
                  ...
                  >
                    ...
                    <TableCell className="py-4 pr-6 text-right" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {user?.role === "super_admin" && (
                            <Button 
                              size="icon" 
                              variant="ghost" 
                              className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg cursor-pointer ml-1" 
                              onClick={(e) => {
                                e.stopPropagation();
                      ...
                      ) : call.status === "team_sent" ? (
                        <div className="flex items-center justify-end gap-2">
                          <Button 
                            size="sm" 
                            className="h-8 px-3.5 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white text-[10px] uppercase font-black tracking-wider rounded-lg shadow-sm shadow-emerald-500/10 cursor-pointer flex items-center gap-1.5" 
                          ...
                          >
                            <SelectTrigger className="w-[115px] h-8 text-[10px] font-black uppercase tracking-wider border-slate-200 bg-white shadow-sm rounded-lg">
                        ...
                        <div className="flex items-center justify-end">
                          ...
                          {user?.role === "super_admin" && (
                            <Button 
                              size="icon" 
                              variant="ghost" 
                              className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg cursor-pointer ml-2" 
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteCall(call.id);
                              }}
        ...
        <div className="md:hidden divide-y divide-gray-100">
          {isLoading ? (
            <div className="text-center py-20 flex flex-col items-center gap-3">
              <Loader2 className="h-8 w-8 animate-spin text-[#ff7a18]" />
              <span className="text-xs font-bold text-gray-400 uppercase">Accessing Database...</span>
            </div>
          ) : !data || data.data.length === 0 ? (
            <div className="text-center py-20 px-6">
              <AlertCircle className="h-10 w-10 text-slate-300 mx-auto mb-3" />
              <h4 className="font-bold text-gray-800 text-sm">No Medical Coordinates</h4>
            ...
            data.data.map((call) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2 font-mono text-xs font-bold text-[#ff7a18]">
                    {call.status === "new" && (
                      <span className="relative flex h-2 w-2 shrink-0">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                        <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
                ...
                <div>
                  <h4 className="font-extrabold text-base text-gray-900 font-['Outfit']">{call.donorName}</h4>
                  <div className="flex items-center gap-2 text-xs text-gray-500 font-semibold mt-1">
                    <span className="bg-slate-100 px-1.5 py-0.5 rounded border border-gray-200/50">{call.donorAge} Years</span>
                    <span>•</span>
                    <span className="capitalize">{call.donorGender}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-y border-gray-50 py-3">
                  <div className="space-y-1">
                    ...
                    <div className="font-bold text-xs text-gray-800 flex items-center gap-1">
                      ...
                      <span className="truncate">{call.district}</span>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Referrer</span>
                    <div className="font-bold text-xs text-gray-800 flex items-center gap-1">
                      <User className="h-3 w-3 text-slate-400 shrink-0" />
                      <span className="truncate">{call.referrerName}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {call.status === "new" && (
                      <Button
                        size="sm"
                        className="h-8 px-3 bg-red-600 hover:bg-red-700 text-white text-[10px] font-black uppercase tracking-wider rounded-lg cursor-pointer"
                        onClick={() => handleStatusChange(call.id, "team_sent")}
                      >
                        Dispatch
                      </Button>
                    ...
                    {call.status === "team_sent" && (
                      <Button
                        size="sm"
                        className="h-8 px-3 bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-wider rounded-lg flex items-center gap-1 cursor-pointer"
                        onClick={() => handleStatusChange(call.id, "completed")}
                      >
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        {selectedCall && (
          <DialogContent className="w-[94vw] max-w-[850px] max-h-[90vh] sm:max-h-[92vh] overflow-y-auto rounded-2xl sm:rounded-3xl bg-white border border-slate-150 p-0 shadow-2xl overflow-x-hidden">
            
            {/* Modal Glass Banner */}
            <div className="p-5 sm:p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-orange-950 text-white flex flex-col sm:flex-row justify-between items-start gap-4 sticky top-0 z-20 shadow-md">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
            ...
            <div className="p-6">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <Card className="shadow-sm border border-slate-150 rounded-2xl p-5 bg-orange-50/20 border-orange-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                      <Select 
                        ...
                        disabled={updateStatus.isPending}
                      >
                        <SelectTrigger className="flex-1 sm:w-[180px] bg-white border-slate-200/80 shadow-sm font-bold text-xs h-10 rounded-xl">
                          <SelectValue placeholder="Update Status" />
                        </SelectTrigger>
                      ...
                      <Button 
                        ...
                        className="flex-1 sm:flex-none bg-slate-900 hover:bg-slate-800 text-white rounded-xl h-10 text-[10px] sm:text-xs font-extrabold uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-md cursor-pointer px-3 sm:px-4"
                      >
                        <Printer className="h-4 w-4 shrink-0" /> <span className="hidden xs:inline">Preview</span><span className="xs:hidden">Print</span>
                      </Button>
                      
                      {isOutOfRegionState(selectedCall.state) && (
                        <Button 
                          onClick={() => {
                            setThirdPartyHospital(selectedCall.thirdPartyHospitalDetails || "");
                            setThirdPartyHelper(selectedCall.thirdPartyHelperContact || "");
                            setIsThirdPartyDialogOpen(true);
                          }}
                          className="flex-1 sm:flex-none bg-[#ff7a18] hover:bg-[#e66a12] text-white rounded-xl h-10 text-[10px] sm:text-xs font-extrabold uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-md cursor-pointer px-3 sm:px-4"
                        >
                          <Building className="h-4 w-4 shrink-0" /> Assign
                        </Button>
                      )}
                    </div>
                  </Card>
                ...
                <TabsContent value="certificate" className="space-y-6 mt-0 animate-fadeIn relative">
                  ...
                  <div className="border border-slate-200 rounded-2xl sm:rounded-3xl bg-slate-50/50 p-3 sm:p-6 flex justify-center no-print max-h-[500px] overflow-y-auto shadow-inner">
                    
                    {/* The actual preview matching print output */}
                    <div className="w-full max-w-[700px] bg-white border-4 border-double border-slate-800 p-4 sm:p-8 shadow-md rounded-sm font-serif text-slate-900 overflow-x-hidden">
                      
                      {/* Logo and Branding header */}
                      <div className="text-center pb-4 border-b-2 border-slate-800">
                        <h2 className="text-base sm:text-lg font-black tracking-wide uppercase text-slate-900">Sri Kanchi Kamakoti Medical Trust</h2>
                        <h3 className="text-sm sm:text-base font-extrabold uppercase text-slate-800 tracking-wider">Sankara Eye Hospitals</h3>
                        <p className="text-[8px] sm:text-[9px] font-sans font-black tracking-widest text-slate-500 uppercase">Corneal Retrieval & Eyebank Division</p>
                        <div className="w-[80px] sm:w-[100px] h-0.5 bg-slate-800 mx-auto my-2" />
                        <h4 className="text-xs sm:text-sm font-black underline uppercase tracking-widest text-slate-950 font-serif">Donor Eye Corneal Retrieval Case Sheet</h4>
                      </div>

                      {/* Grid 1: Meta data */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-[10px] sm:text-xs py-4 border-b border-slate-600">
                        <div>
                        ...
                        <div className="sm:text-right">
                          ...
                          <p className="mt-1"><strong>Status:</strong> <span className="uppercase text-[9px] sm:text-[10px] font-sans font-black tracking-wider">{selectedCall.status}</span></p>
                        </div>
                      </div>
      ...
      <Dialog open={isThirdPartyDialogOpen} onOpenChange={setIsThirdPartyDialogOpen}>
        <DialogContent className="sm:max-w-[425px] rounded-3xl border-0 shadow-2xl p-0 overflow-hidden bg-white">
          <div className="p-6 pb-2 border-b border-slate-100">
            <DialogHeader>
              <DialogTitle className="text-lg font-black text-slate-800 uppercase tracking-wider flex items-center gap-2">
                <Building className="h-5 w-5 text-[#ff7a18]" /> Assign Regional Partner
              </DialogTitle>
              <DialogDescription className="text-xs text-slate-500 font-semibold mt-2">
                Provide details for the third-party NGO or Hospital taking over this retrieval. The primary referrer will be notified instantly via SMS.
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="p-6 space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Hospital/NGO Name</label>
              <Input 
                value={thirdPartyHospital}
                onChange={(e) => setThirdPartyHospital(e.target.value)}
                placeholder="e.g., Lions Eye Bank Hospital"
                className="h-10 rounded-xl bg-slate-50 border-slate-200"
              />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Helper Contact Number</label>
              <Input 
                value={thirdPartyHelper}
                onChange={(e) => setThirdPartyHelper(e.target.value)}
          ...
          <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
            <Button 
              variant="outline" 
              onClick={() => setIsThirdPartyDialogOpen(false)}
            ...
            <Button 
              ...
              disabled={updateCall.isPending || !thirdPartyHospital || !thirdPartyHelper}
              className="bg-[#ff7a18] hover:bg-[#e66a12] text-white rounded-xl h-10 text-xs font-extrabold uppercase tracking-wider"
            >
              {updateCall.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save & Notify"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
    ...
    {selectedCall && (
                  <div id="printable-case-sheet" className="hidden print:block p-8 border-4 border-double border-slate-900 bg-white font-serif max-w-[800px] mx-auto text-slate-900">
                    ...
                    <div className="py-4 border-b border-slate-600 text-xs">
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        <tbody>
                          ...
                          {selectedCall.thirdPartyHospitalDetails && (
                            <tr>
                              <td className="py-1"><strong>Third Party Partner Hospital:</strong></td>
                              <td className="py-1 font-sans font-bold text-amber-900">{selectedCall.thirdPartyHospitalDetails}</td>
                            </tr>
                          )}
                          {selectedCall.thirdPartyHelperContact && (
                            <tr>
                              <td className="py-1"><strong>Partner Helper Contact:</strong></td>
                              <td className="py-1 font-sans font-bold text-amber-900">{selectedCall.thirdPartyHelperContact}</td>
                            </tr>
                          )}
